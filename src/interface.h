...
class quick_sim_interface
{
  public:
    ...
    quick_sim_interface(std::string t_in_path, std::string t_out_path, bool verbose = true, int log_l = Logger::MSG) :
            log_level{log_l},
            in_path(std::move(t_in_path)),
            out_path(std::move(t_out_path))
    {
        ...
        for (const auto& [energy, deg] : energy_occ_distribution)
        {
            for (const auto& lyt : sim_results.valid_lyts)
            {
                if (std::abs(energy - lyt.get_system_energy()) < physical_constants::POP_STABILITY_ERR)
                {
                    ...
                    db_dist.push_back(charge_configuration_to_string(lyt.get_all_sidb_charges()));  // config
